...
from com_luwen import louvain_communities
...
def get_com():
    ...
    if len(nets) > 1:
        ...
        com = louvain_communities(lgst_net, k=num_local)
    else:
        com = louvain_communities(net, k=num_local)
    anchor_nodes = []
    node2cluster = {}
    node2localcom = {}
    nums = 0
    ...
    for j, local_c in enumerate(com):
        start_time = time.time()
        local_subnet = nx.subgraph(net, local_c)
        # print("here")
        # 度中心性
    ...
    lap_pe = positional_encoding(landmark_graph.edge_index, len(com))
    lap_pe = np.concatenate([lap_pe, np.zeros(len(com) - 1).reshape(1, -1)], axis=0)

    lap_pe = lap_pe[membership]
    print('lap_pe=',lap_pe.shape)
...
def main():
    ...
    for pidx in range(WORKER_NUM):
        p = multiprocessing.Process(target=get_pe, args=(
        net, anchor_nodes, nodes[pidx * bs: min((pidx + 1) * bs, len(nodes))], return_dict))
        p.start()
