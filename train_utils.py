...
def get_train_test(args):
    if args.dataset == "ogbl-citation2":
        ...
        return train_hits, test_hits, evaluator, loggers, loggers2


def constraint_loss_with_M(v, M, tau=0.5):
    M = M.coalesce()
    if M._nnz() == 0:
    ...
    sim = F.cosine_similarity(v[row], v[col])

    exp_sim = torch.exp(sim / tau)
    denom = torch.zeros(v.size(0), device=v.device).index_add_(0, row, exp_sim)
    prob = exp_sim / (denom[row] + 1e-8)

    loss = - (weights * torch.log(prob + 1e-8)).mean()
    return loss

def train_hits(encoder, predictor, data, split_edge, optimizer, batch_size,
            mask_target, dataset_name, num_neg, adj2, mode='tea', teacher_encoder=None, top_10=None, last_10=None,
               tau=0.5, lambda_con=0.4):
    encoder.train()
    predictor.train()
    ...
    if dataset_name == "ogbl-vessel":
        ...
        #neg_edge_epoch = negative_sampling(data.custom_edge_index, num_nodes=data.adj_t.size(0),
        neg_edge_epoch = negative_sampling(data.edge_index, num_nodes=data.adj_t.size(0),
                                           num_neg_samples=num_pos_max * num_neg)
    else:
        #neg_edge_epoch = negative_sampling(data.custom_edge_index, num_nodes=data.adj_t.size(0),
        neg_edge_epoch = negative_sampling(data.edge_index, num_nodes=data.adj_t.size(0),
    ...
    for perm in tqdm(DataLoader(range(pos_train_edge.size(0)), batch_size,
        ...
        if mode == 'tea':
            h = encoder(data.x, adj_t, data['pos_enc'], data['lap_pe'], mode=mode)
        elif mode == 'stu':
            h = encoder(data.x, adj_t, mode=mode)
        ...
        out = predictor(h, adj_t, train_edges, node2local=data['node2local'], center_dis=data.get('center_dis', None), adj2 = adj2).squeeze()

        #anchor_h = h[data['node2anchor']]
        #loss_per_node = torch.mean((h - anchor_h) ** 2, dim=1)  # shape: (num_nodes,)
        #anchor_loss = torch.mean(loss_per_node)

        #loss = criterion(out, train_label)
        #print('loss=',loss)
        #print('anchorloss=',anchor_loss)
        #batch_nodes = torch.unique(train_edges.flatten())  # 锟斤拷前 batch 锟斤拷锟芥及锟斤拷锟斤拷锟叫节碉拷锟斤拷锟斤拷
        #anchor_indices = data['node2anchor'][batch_nodes]  # 锟斤拷应 anchor 锟节碉拷锟斤拷锟斤拷

        #batch_h = h[batch_nodes]
        #anchor_h = h[anchor_indices]

        #anchor_loss = F.mse_loss(batch_h, anchor_h, reduction='mean')  # 只锟斤拷 batch 锟斤拷锟斤拷
        #loss += 0.4*anchor_loss


        #if mode == 'stu':
            #z = teacher_encoder(data.x, adj_t, data['pos_enc'], data['lap_pe'], mode='tea')
            #loss += 10*F.mse_loss(h, z)

        # ===== BCE loss =====
        loss_bce = criterion(out, train_label)

        # ===== Constraint loss =====

        L_con = constraint_loss_with_M(h, data['M'], tau=0.5)

        # ===== Total loss =====
        #print('loss_bce=', loss_bce)
        #print('L_con=', L_con)
        loss = loss_bce + lambda_con * L_con
...
def test_hits(encoder, predictor, data, split_edge, evaluator, 
         batch_size, use_valedges_as_input, fast_inference, adj2, mode='tea', top_10=None, last_10=None):
    encoder.eval()
    predictor.eval()
    device = data.adj_t.device()
    adj_t = data.adj_t
    ...
    if mode == 'tea':
        h = encoder(data.x, adj_t, data['pos_enc'], data['lap_pe'])
    elif mode == 'stu':
        h = encoder(data.x, adj_t, mode=mode)
    ...
    def test_split(split, cache_mode=None):
        ...
        for perm in DataLoader(range(pos_test_edge.size(0)), batch_size):
            ...
            out = predictor(h, adj_t, edge, node2local=data['node2local'], cache_mode=cache_mode, adj2 = adj2)
        ...
        for perm in DataLoader(range(neg_test_edge.size(0)), batch_size):
            ...
            neg_test_preds += [predictor(h, adj_t, edge, node2local=data['node2local'], cache_mode=cache_mode, adj2 = adj2).squeeze().cpu()]
...
def test_mrr(encoder, predictor, data, split_edge, evaluator, 
    ...
    def test_split(split, cache_mode=None):
        ...
        for perm in DataLoader(range(source.size(0)), batch_size):
            ...
            pos_preds += [predictor(h, adj_t, torch.stack((src, dst)), cache_mode=cache_mode, center_dis=data.get('center_dis', None), adj2 = adj2).squeeze().cpu()]
        ...
        for perm in DataLoader(range(source.size(0)), batch_size):
            ...
            neg_preds += [predictor(h, adj_t, torch.stack((src, dst_neg)), cache_mode=cache_mode, center_dis=data.get('center_dis', None), adj2 = adj2).squeeze().cpu()]
